...
router.post("/autogit/generate", async (req, res) => {
  ...
  const ownerId = String(req.session.githubId ?? req.sessionID);
  sseWrite(res, { type: "session", sessionId });

  try {
    await db.insert(scaffoldsTable).values({ id: sessionId, githubId: ownerId, prompt, files: [], status: "generating" });
...
router.post("/autogit/improve", async (req, res) => {
  ...
  const ownerId = String(req.session.githubId ?? req.sessionID);
  try {
    const [session] = await db.select().from(scaffoldsTable).where(and(eq(scaffoldsTable.id, sessionId), eq(scaffoldsTable.githubId, ownerId)));
    if (!session) {
...
router.get("/autogit/sessions", async (req, res) => {
  const ownerId = String(req.session.githubId ?? req.sessionID);
  try {
    const rows = await db.select().from(scaffoldsTable).where(eq(scaffoldsTable.githubId, ownerId)).orderBy(desc(scaffoldsTable.createdAt)).limit(20);
...
router.get("/autogit/sessions/:sessionId", async (req, res) => {
  const ownerId = String(req.session.githubId ?? req.sessionID);
  try {
    const sid = String(req.params["sessionId"] ?? "");
    const [row] = await db.select().from(scaffoldsTable).where(and(eq(scaffoldsTable.id, sid), eq(scaffoldsTable.githubId, ownerId)));
...
router.post("/autogit/deploy", requireAuth, async (req, res) => {
  ...
  try {
    const [session] = await db.select().from(scaffoldsTable).where(and(eq(scaffoldsTable.id, sessionId), eq(scaffoldsTable.githubId, String(req.session.githubId!))));
