...
const GITBANK_AIRDROP    = "0xAa29D8644EB53796eE123b09e3D9177CC99C480f"; // batch push-airdrop deployer is owner
...
function buildV4SwapData(tokenOut, amountIn, recipient, poolInfo) {
  ...
  // TAKE_ALL: only (currency, minAmount) — NO recipient param.
  // Tokens go to msg.sender automatically.
...
async function sendTx(to, data, value = 0n) {
  // Use module-level nonce counter to avoid RPC staleness race conditions
  const nonce = await getNextNonce();
  // Use 20% above current gas price to replace any stuck pending txs with same nonce
  ...
  const hash = await walletClient.sendTransaction({ to, data, value, gas, gasPrice, nonce });
  const receipt = await confirm(hash);
  if (receipt.status !== "success") throw new Error(`tx reverted: ${hash}`);
  return hash;
}

// Like sendTx but with an explicit gas limit (for large batch txs where estimateGas may fail)
async function sendTxWithGas(to, data, value = 0n, gas) {
  const nonce = await getNextNonce();
  const baseGasPrice = await publicClient.getGasPrice();
  const gasPrice = (baseGasPrice * 120n) / 100n;
  const hash = await walletClient.sendTransaction({ to, data, value, gas, gasPrice, nonce });
...
// Module-level nonce counter — fetched once at start, incremented after each confirmed tx.
// Avoids RPC nonce staleness race condition.
let _nonce = null;
async function getNextNonce() {
  if (_nonce === null) {
    _nonce = await publicClient.getTransactionCount({ address: deployer, blockTag: "pending" });
  }
  return _nonce++;
}
...
async function run() {
  ...
  // If deployer already holds the token (from a previous partial run), skip the swap step.
  const _resumeToken = process.env.TOKEN_ADDRESS ?? null;
  const existingTokenBal = _resumeToken
    ? await publicClient.readContract({ address: _resumeToken, abi: ERC20_ABI, functionName: "balanceOf", args: [deployer] }).catch(() => 0n)
    : 0n;
  const skipSwap = existingTokenBal > 0n;
  if (skipSwap) {
    pass("Existing token balance: " + (Number(existingTokenBal)/1e18).toFixed(4) + " — skipping swap, distributing existing tokens");
  }

  // Preflight: need ETH + WETH to cover the auto-buy (only if not skipping swap).
  const existingWethPre = await publicClient.readContract({ address: WETH, abi: WETH_ABI, functionName: "balanceOf", args: [deployer] });
  pass("WETH balance: " + formatEther(existingWethPre) + " (counts toward auto-buy)");
  if (!skipSwap) {
    const ethNeededForBuy = existingWethPre >= AUTO_BUY_ETH ? 0n : AUTO_BUY_ETH - existingWethPre;
    const minEthNeeded = ethNeededForBuy + 1_000_000_000_000_000n; // buy + 0.001 ETH gas buffer (Base is cheap)
    if (ethBal < minEthNeeded) {
      fail("Deployer ETH too low — have " + formatEther(ethBal) + " ETH, need " + formatEther(minEthNeeded) + " (WETH covers " + formatEther(existingWethPre) + " of the buy)");
      process.exit(1);
    }
    pass("ETH + WETH sufficient for auto-buy ✅");
  } else {
    pass("Gas only check: " + formatEther(ethBal) + " ETH available for gas ✅");
  }

  // ── 1. Deploy TEST token via Clanker v4 (or resume existing) ──────────────
  section("1. Deploy " + TOKEN_NAME + " via Clanker v4");

  // Accept pre-deployed token via env vars to make the script resumable.
  // Usage: TOKEN_ADDRESS=0x... DEPLOY_TX=0x... node launch-token-e2e.mjs
  let tokenAddress = process.env.TOKEN_ADDRESS ?? null;
  let deployTxHash = process.env.DEPLOY_TX ?? null;

  if (tokenAddress && deployTxHash) {
    pass("Resuming with existing token: " + tokenAddress);
    pass("Deploy tx: " + deployTxHash);
    ...
    const { txHash: dTx, waitForTransaction, error: deployError } = await clanker.deploy(tokenConfig);
    ...
    const { address: addr, error: waitError } = await waitForTransaction();
    if (waitError) { fail("waitForTransaction error: " + waitError); process.exit(1); }
    if (!addr) { fail("No token address returned"); process.exit(1); }
    tokenAddress = addr;
  ...
  if (skipSwap) {
    info("Skipping swap — deployer already holds " + (Number(existingTokenBal)/1e18).toFixed(4) + " TEST tokens");
    info("These will be distributed to all $GITBANK holders");
  } else {
    // Check if we already have WETH from a previous run (test is resumable)
    const existingWeth = await publicClient.readContract({ address: WETH, abi: WETH_ABI, functionName: "balanceOf", args: [deployer] });
    if (existingWeth >= AUTO_BUY_ETH) {
      info("Already have " + formatEther(existingWeth) + " WETH — skip wrap");
    } else {
    ...
    info("Step 3b: WETH.approve(Permit2) — V4 UniversalRouter uses Permit2 for token pulls");
    const approvePermit2Data = encodeFunctionData({
      abi: WETH_ABI, functionName: "approve",
      args: [PERMIT2, AUTO_BUY_ETH],
    });
    const approveTx = await sendTx(WETH, approvePermit2Data);
    addTx("WETH approve Permit2", approveTx);

    info("Step 3c: Permit2.approve(WETH, V4Router) — grant router allowance via Permit2");
    ...
    const permit2Data = encodeFunctionData({
      ...
      args: [WETH, UNISWAP_V4_ROUTER, AUTO_BUY_ETH, expiration],
    });
    const permit2Tx = await sendTx(PERMIT2, permit2Data);
    addTx("Permit2 approve V4 router", permit2Tx);

    info("Step 3d: V4 swap WETH → TEST via UniversalRouter");
  ...
  // ── 5. Distribute TEST tokens to all $GITBANK holders via batch airdrop ─
  ...
  info("Method: GitbankAirdrop.batchTransfer — " + AIRDROP_BATCH_SIZE + " holders per tx");
  info("Airdrop contract: " + GITBANK_AIRDROP);

  // Build recipient + amount arrays
  const recipients = [];
  const amounts = [];
  let skipped = 0;
  for (const [addr, gitbankBal] of holders) {
    const share = (tokensBought * gitbankBal) / totalGitbank;
    if (share === 0n) { skipped++; continue; }
    recipients.push(addr);
    amounts.push(share);
  }
  info("Recipients with non-zero share: " + recipients.length + " (skipped " + skipped + " zero-share)");

  // Step 5a: approve airdrop contract for total
  const totalToSend = amounts.reduce((s, a) => s + a, 0n);
  info("Step 5a: token.approve(GitbankAirdrop, " + (Number(totalToSend)/1e18).toFixed(4) + ")");
  const tokenApproveData = encodeFunctionData({ abi: ERC20_ABI, functionName: "approve", args: [GITBANK_AIRDROP, totalToSend] });
  const tokenApproveTx = await sendTx(tokenAddress, tokenApproveData);
  addTx("Token approve airdrop contract", tokenApproveTx);
  pass("Approve tx: " + tokenApproveTx);

  // Step 5b: chunked batchTransfer
  const batchTxs = [];
  const totalBatches = Math.ceil(recipients.length / AIRDROP_BATCH_SIZE);
  for (let b = 0; b < totalBatches; b++) {
    const start = b * AIRDROP_BATCH_SIZE;
    const end = Math.min(start + AIRDROP_BATCH_SIZE, recipients.length);
    const batchRecipients = recipients.slice(start, end);
    const batchAmounts = amounts.slice(start, end);
    info(`  Batch ${b+1}/${totalBatches}: sending to ${batchRecipients.length} holders...`);
    const batchData = encodeFunctionData({
      abi: AIRDROP_ABI,
      functionName: "batchTransfer",
      args: [tokenAddress, batchRecipients, batchAmounts],
    });
    // Gas: ~25k per ERC20 transferFrom (cold SSTORE) + 1M overhead for calldata + loop
    const batchGas = BigInt(batchRecipients.length) * 28_000n + 1_200_000n;
    const batchTx = await sendTxWithGas(GITBANK_AIRDROP, batchData, 0n, batchGas);
    batchTxs.push(batchTx);
    addTx("Airdrop batch " + (b+1), batchTx);
    pass(`  Batch ${b+1}/${totalBatches}: ${EXPLORER}/${batchTx}`);
  }

  const distributed = recipients.length;
  pass("Distribution complete!");
  pass("Distributed to: " + distributed + " holders via " + batchTxs.length + " batch txs");
  ...
  if (batchTxs && batchTxs.length > 0) {
    console.log("\n  BATCH AIRDROP TXs (" + batchTxs.length + " batches):");
    for (const hash of batchTxs) {
      console.log("  " + EXPLORER + "/" + hash);
    }
  }
